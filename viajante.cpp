...
void dijkstra_distancias(int inicial){
    ...
    while(!fila.empty()){ // Enquanto nao fila vazia
        ...
        int topline = fila.top();

        fila.pop();
        int i = 0;
        for(auto v: grafo[topline]){ // Visitamos seus vizinhos
            i++;
            // Se a distancia do vertice atual pro vizinho for menor que a do inicial pro vizinho, atualizar
            //
            cout<<v.first<<i<<endl;

            if(v.first + distancia[inicial][topline] < distancia[inicial][v.second]){

                distancia[inicial][v.second] = distancia[inicial][topline] + v.first;
...
//-------------------------------//
    ...
    for(int i = 0; i < V; i++){ // Para cada vertice ligado ao 'vertice'
        ...
        if(i != vertice and (!((bitmask>>i) & 1) or i == Base)){
